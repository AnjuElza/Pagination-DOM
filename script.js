...
let totalPages= Math.ceil(state.querySet.length/state.rows);
let data=pagination(state.querySet, state.page, state.rows);

//Function for pagination: to set the data on each page
...
//Function to display json data in a html table format
function displayDetails(data){
  let placeholder=document.querySelector("#table-body");
  let detailList=data.querySet;
  let out="";
...
//Function  to display the page numbers
function pageButtons(pages){
  let wrapper=document.getElementById('pagination-wrapper');
  wrapper.innerHTML='';
  for (let page=1;page<=pages;page++){
    wrapper.innerHTML+= `<button value=${page} class="pageButton btn btn-outline-primary btn-lg">${page}</button>`;
...
//To select the data to display on each page
let buttons= document.querySelectorAll('.pageButton');
Array.from(buttons).forEach((button)=>{
    button.addEventListener('click',(e)=>{
        pageNo=e.target.innerHTML;
        let data1=pagination(state.querySet, pageNo, state.rows);
        //console.log(data1);
        displayDetails(data1);
        })
    })
